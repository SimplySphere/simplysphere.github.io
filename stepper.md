...
- The design has 5 components: the mount, the right hemisphere, the left hemisphere, the axis, and the base. The mount is stuck into sockets on both hemispheres and has holes to hold screws for the scanner. The right hemisphere is where the rest of the shaft of the horizontally facing servo is inserted into. The left hemisphere is where the horizontally facing servo is contained, with a cut-out at the bottom for the cables to pass through. The axis is a stick with a ball at the top that has another shorter stick going through it. The axis has the holders for the servos' shaft to be inserted into. Finally, the base is used to balance the servos and the hemispheres. Most of it is hollow to allow for cables, and it has a hole to the side to connect the cables to the Arudino outside. 
